...
pub fn clone_repo(git_url: &str, into_dir: &str) -> Result<()> {
  ...
  // 解析 git url
  let url = Url::parse(git_url).with_context(|| format!("{} 不是有效的git仓库地址。", git_url))?;
...
pub fn clone_private_repo(
  ...
  callbacks.credentials(|_url, _username, _password| Cred::userpass_plaintext(username, password));
