...
impl<'a> TsApiInfo<'a> {
  ...
  fn add_import(&mut self) {
    self
      .script_ast
      .add_import_default("@/utils/request", "request");

    let params_type_name =
      self.alloc_str(format!("{}QueryParams", self.page_key.to_upper_camel_case()).as_str());
    let table_type_name = self.alloc_str(
      self
        .db_table_structure
        .table
        .to_entity_class_name()
        .as_str(),
    );
    self.script_ast.add_import_named_type(
      "@/types",
      &[
        "AjaxResult",
        "TableDataInfo",
  ...
  fn add_fetch_data_list_api(&mut self) {
    ...
    let formal_parameter_type_name =
      self.alloc_str(format!("{}QueryParams", self.page_key.to_upper_camel_case()).as_str());
    let url = self.alloc_str(
      format!(
        "/{}/{}/list",
        self.parent_keys.join("/"),
        self.page_key.to_lower_camel_case()
      )
      .as_str(),
    );
    ...
    self.script_ast.add_api_function(
      ...
      [self
        .script_ast
        .new_formal_type_parameter("params", formal_parameter_type_name)],
      &["TableDataInfo", "DemoTable[]"],
      [self
        .script_ast
        .new_return_request_get_statement(url, &["params"])],
    );
  }
  ...
  fn add_fetch_one_data_api(&mut self) {
    ...
    self.script_ast.add_api_function(
      ...
      [self.script_ast.new_return_request_get_statement(url, &[])],
    );
  }
  ...
  fn add_add_one_data_api(&mut self) {
    ...
    self.script_ast.add_api_function(
      ...
      [self
        .script_ast
        .new_return_request_post_statement(url, "data")],
    );
  }
  ...
  fn add_update_one_data_api(&mut self) {
    ...
    self.script_ast.add_api_function(
      ...
      [self
        .script_ast
        .new_return_request_put_statement(url, "data")],
    );
  }
  ...
  fn add_delete_data_list_api(&mut self) {
    ...
    self.script_ast.add_api_function(
      function_name,
      [self.script_ast.new_formal_union_types_parameter(
        "id",
        [
      ...
      &["AjaxResult"],
      [self.script_ast.new_return_request_delete_statement(url)],
    );
  }
}
...
mod tests {
  use super::*;
  use crate::db_types::{DbColumn, DbDataType, DbTable, DbTableStructure};
