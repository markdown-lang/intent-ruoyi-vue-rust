...
pub fn get_type_code(
  ...
  if let Some(page_key) = page_key {
    let interface_name =
      allocator.alloc_str(format!("{}QueryParams", page_key.to_upper_camel_case()).as_str());
    ...
    for query_param in query_params {
      match query_param.operation {
        MatchOperation::Between => {
          let begin_field_name = allocator
            .alloc_str(format!("begin{}", query_param.column_name.to_upper_camel_case()).as_str());
          let end_field_name = allocator
            .alloc_str(format!("end{}", query_param.column_name.to_upper_camel_case()).as_str());
          if query_param.data_type.is_ts_string_type() {
            params_properties
              .push(script_ast.new_interface_property_string(begin_field_name, true));
            ...
            params_properties
              .push(script_ast.new_interface_property_number(begin_field_name, true));
        ...
        _ => {
          let field_name =
            allocator.alloc_str(query_param.column_name.to_lower_camel_case().as_str());
    ...
    direct_properties.push(script_ast.new_interface_property_type_literal(
      "params",
      params_properties,
      true,
    ));

    script_ast.add_interface(interface_name, direct_properties, &["PageDomain"]);
  }

  // 实体 interface 源码
  // 主表
  let entity_class_name = main_table.table.to_entity_class_name();
  let properties: Vec<TSSignature> = main_table
    .columns
    .iter()
    ...
    .chain(sub_tables.iter().map(|sub_table| {
      ...
      let field_name =
        allocator.alloc_str(format!("{}List", sub_table.table.to_entity_field_name()).as_str());
      script_ast.new_interface_property_array_type(field_name, class_name, true)
    }))
    .collect();

  script_ast.add_interface(entity_class_name.as_str(), properties, &["BaseEntity"]);

  // 从表
  for sub_table in sub_tables {
    let entity_class_name = allocator.alloc_str(sub_table.table.to_entity_class_name().as_str());
    let properties: Vec<TSSignature> = sub_table
      .columns
      .iter()
      .filter(|column| !column.is_audit_field())
      .map(|column| {
      ...
      .collect();
    script_ast.add_interface(entity_class_name, properties, &["BaseEntity"]);
  }
...
mod tests {
  use super::*;
  use crate::db_types::{DbColumn, DbDataType, DbTable, MatchOperation, QueryParam};
  ...
  fn test_get_type_code_sub_tables() {
    ...
    let actual_code = get_type_code(
      main_table_structure,
      vec![sub_table_structure_1, sub_table_structure_2],
      None,
      vec![],
    );
    let expect_code = concat!(
